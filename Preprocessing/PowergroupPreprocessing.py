...
class PowergroupPreprocessing:
    """
        ...
        :param multi_csv: The number of power csv file may be multiple, seperated in the newer version
        the parameter is set to deal with the situation if multiple csv files needed to be concatenated
        before the next step of preprocessing
        :type multi_csv: bool
    """
    ...
    def load(self):
        ...
        Add: if argument 'multi-csv' is True, meaning there is a lot of
        csv file together, the data is concatenated first before entering
        the preprocessing
        """
        print('[INFO] Read original power data')
        if self.multi_csv:
            filenames = os.listdir(self.power_data_csv_path)
            for file in filenames:
                if file.endswith(".csv"):
                    df = pd.read_csv(os.path.join(self.power_data_csv_path, file), dtype={'terrainID': str})
                    self.original_data = pd.concat([self.original_data, df], ignore_index=True, axis=0)

            self.original_data.drop("current_height", axis=1, inplace=True)
            self.original_data.drop("image_height", axis=1, inplace=True)
        else:
            ...
            for root, dirs, files in os.walk(self.src_folder_path):
                for filename in files:
                    if any(filename.endswith(extension) for extension in self.image_extension):
                        ...
                        if df.empty:
                            print(
                                "[WARNING] No corresponding power data has been found in the given CSV file: {}".format(
                                    filename))
    ...
    def labeling(self):
        ...
        self.labelled_data.to_csv(self.processed_data_file, index=None)
