...
class EnergyPredictorV2_image2heightmap():
    ...
    def start(self):
        ...
        sys.stdout = Logger.Logger("result_i2h_{}.txt".format(time.strftime('%Y-%m-%d_%H-%M-%S', self.time_start)))
...
class EnergyPredictorV2_heightmap2powerclass():
    def __init__(self,
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='max', factor=0.7, patience=5,
    ...
    def train(self, cur_epoch):
        ...
        print("\n[INFO] Epoch {}/{} average training loss: {}, accuracy: {}/{} ({:.4f}%)\n".format(cur_epoch,
                                                                                                self.num_epochs,
    ...
    def test(self, cur_epoch):
        ...
        self.scheduler.step(num_correct / len(test_loader.dataset) * 100.0)
        self.check_optim(test_loss_sum / len(test_loader.dataset))
    ...
    def start(self):
        self.time_start = time.localtime()
        # log the file into result
        sys.stdout = Logger.Logger("result_h2c_{}.txt".format(time.strftime('%Y-%m-%d_%H-%M-%S', self.time_start)))
        for epoch in range(1, self.num_epochs+1):
            self.train(epoch)
            self.test(epoch)


class EnergyPredictorV2_image2powerclassification:
    def __init__(self,
                 train_src_folder_path: str,
                 test_src_folder_path: str,
                 classification_datafile_src_path: str,
        ...
        self.trainset = ImageDataset.Image2PowerclassDataset(
            img_src_folder_path=self.train_src_folder_path,
            classification_datafile_src_path=self.classification_datafile_src_path,
            transform=self.img_transforms['train']
        )
        self.testset = ImageDataset.Image2PowerclassDataset(
            img_src_folder_path = self.test_src_folder_path,
            classification_datafile_src_path=self.classification_datafile_src_path,
            transform=self.img_transforms['test']
        )
    ...
    def start(self):
        self.time_start = time.localtime()
        # log the file into result
        sys.stdout = Logger.Logger("result_i2c_{}.txt".format(time.strftime('%Y-%m-%d_%H-%M-%S', self.time_start)))
        for epoch in range(1, self.num_epochs+1):
            self.train(epoch)
            self.test(epoch)
